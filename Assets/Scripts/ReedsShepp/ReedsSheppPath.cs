    ...
    public class ReedsSheppPath
    {
        ...
        private Tuple<bool, double, double, double> straight_left_straight(double x, double y, double phi)
        {
            ...
            if (y > 0.0 && 0.0 < phi && phi < Math.PI * 0.99)
            {
                ...
                double t = xd - Math.Tan(phi / 2.0);
                double u = phi;
                double v = Math.Sqrt(Math.Pow((x - xd), 2) + Math.Pow(y, 2)) - Math.Tan(phi / 2.0);
        ...
        private List<Path> set_path(List<Path> paths, List<double> lengths, List<string> ctypes, double step_size, string whoAreYou)
        {
            Path path = new Path();
            path.ctypes = ctypes;
            ...
            {
                bool type_is_same = equalPathTypes(i_path.ctypes, path.ctypes);
        ...
        private bool equalPathTypes(List<string> path1_ctypes, List<string> path2_ctypes)
        {
        ...
        private Tuple<bool, double, double, double> left_right_left(double x, double y, double phi)
        {
            ...
            if (u1_t1.Item1 <= 4.0)
            {
                ...
                double t = mod2pi(u1_t1.Item2 + 0.5 * u + Math.PI);
        ...
        private List<List<double>> calc_interpolate_dists_list(List<double> lengths, double step_size)
        {
            ...
            foreach (double length in lengths)
            {
                ...
                var interp_dists = Generate.LinearRange(0.0, d_dist, length); // Return evenly spaced values from within a given interval. Param1 -> Start of interval, Param2 -> Steps/Spacing between intervals, Param3 -> End of interval.
        ...
        private List<Path> calc_paths(double sx, double sy, double syaw, double gx, double gy, double gyaw, double maxc, double step_size)
        {
        ...
        public Tuple<List<double>, List<double>, List<double>, List<string>, List<double>> reeds_shepp_path_planning(double sx, double sy, double syaw, double gx, double gy, double gyaw, double maxc, double step_size = 0.5)
        {
            List<Path> paths = calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size);
            if (paths == null || paths.Count < 1)
            {
                return new Tuple<List<double>, List<double>, List<double>, List<string>, List<double>>(null, null, null, null, null); // could not generate any path
            }
            // search minimum cost path
            var minLengthPath = findMinLengthPath(paths);
            int best_path_index = paths.IndexOf(minLengthPath);
            Path b_path = paths[best_path_index];
            return new Tuple<List<double>, List<double>, List<double>, List<string>, List<double>>(b_path.x, b_path.y, b_path.yaw, b_path.ctypes, b_path.lengths);
        }

        private Path findMinLengthPath(List<Path> list)
        {
            Path min = list[0];
